...
function usage()
{
    echo 'Usage: ssh-target.sh add <SERVER_NAME> <SERVER_IP>'
...
function ssh-target-add
{
    ...
    # add record
    sed -i '/BEGIN_OF_IP_LIST/a'"SERVER_${SERVER_NAME}_IP=\"${SERVER_IP}\"" "${IP_MAP_FILE}"
    ...
    if [ "${RES}" == "" ]; then
        ...
        echo "Server ${SERVER_NAME} already exist, modified to ${SERVER_IP} succeeful."
    else
        echo "Server ${SERVER_NAME} => ${SERVER_IP} added successful."
    fi
}
...
{
    grep -P "SERVER_[^_]+_IP=" "${IP_MAP_FILE}" | sed -r -e 's/SERVER_([^_]+)_IP=(.+)/\1\t=> \2/g' -e 's/"//g'
}
